...
typedef enum
{
	...
	MOTOR_Rack_StandBy = 0x01, //样本架入轨辅助电机
	MOTOR_Rack_Convey = 0x02,    //样本架传送轨道驱动电机
	MOTOR_Tube_Rolling = 0x03, //样本管条码阅读辅助驱动电机	
	MOTOR_Puncture_Limit = 0x04, //穿刺限位电机
	MOTOR_Rack_Out = 0x05,  //样本架出轨驱动电机
    MOTOR_TUBE_SHAKING_Z = 0x06,//摇匀模块Z轴电机 
    MOTOR_TUBE_CLAMP = 0x07,//摇匀夹取电机
    MOTOR_TUBE_SHAKING = 0x08,//摇匀电机       
	#endif
	//拨卡模块
	#ifdef Card_Transfer_Assembly
	MOTOR_Stripe_Out1 = 0x00,    //卡条出盒驱动电机1
	MOTOR_Stripe_Out2 = 0x01,    //卡条出盒驱动电机2
	MOTOR_Stripe_Out3 = 0x02,  //卡条出盒驱动电机3
	MOTOR_Stripe_Out4 = 0x03, //卡条出盒驱动电机4	
	MOTOR_Stripe_Out5 = 0x04, //卡条出盒驱动电机5
	MOTOR_Stripe_Convey1 = 0x05, //卡条进入换向机构驱动电机
	MOTOR_Stripe_realignment = 0x06,   //卡条换向驱动电机
	MOTOR_Stripe_Convey2 = 0x07,  //卡条送入检测盘驱动电机
	#endif
	//移液模组
	...
	MOTOR_Signal_Scan = 0x07,  //采光头扫描电机
	#endif	
...
typedef enum
{
	...
	CMD_SEND_SAMPLE_STATUS = 0x03,       //发送样本信息
	CDM_PREPARE_NEXT_SAMPLE = 0x04,     //准备穿刺下一个样品
	CMD_RACK_OUT = 0x05,         //出试管架  
	CDM_CONTINUE_TEST = 0x06,       //继续检测	
	CMD_TUBE_ERROR = 0x07,   
    CMD_GET_SAMPLE_TYPE = 0x0A,         //接收样本信息
	...
	CMD_SCAN_ADD_ITEMS = 0x46,                   //扫码添加项目
	CMD_START_AGING = 0x60,              //开始老化
	CMD_STOP_AGING = 0x61,              //停止老化
	CMD_UPDATE_APP = 0x62,             //更新固件
...
typedef enum
{
	...
	RACK_OUT = 0x05,                 //出试管架 
	CONTINUE_TEST = 0x06,       //继续检测	
    TUBE_ERROR = 0x07,
	SEND_SAMPLE_NUMB = 0x08,    //发送试管架码
	SEND_SAMPLE_SCANERR = 0x09,    //发送没扫到试管架码
    GET_SAMPLE_TYPE = 0x0A,         //接收样本信息
	PUSH_CARD = 0x11,             //拨卡
	...
	SCAN_ADD_ITEMS = 0x46,                   //扫码添加项目
	GET_WRONG_SCALING_MSG = 0x47,        //获取到错误的外扫码数据
	START_AGING = 0x60,              //开始老化
	STOP_AGING = 0x61,              //停止老化
	UPDATE_APP = 0x62,             //更新固件	
...
typedef __packed struct
{
    U8  mainVersion;        //主版本
    U8  childVersion;       //子版本
    U8  reviseVersion;      //修订版本
    U8  buildVersion;       //编译版本
}SOFT_VERSION;
