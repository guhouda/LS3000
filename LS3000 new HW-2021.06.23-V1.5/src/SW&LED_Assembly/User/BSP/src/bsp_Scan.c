...
SCAN_PARAM Scan_Param[MAX_SCAN_NUM];
//条码命令
...
_Bool MJ2000_In_Init(U8 uartId)  
{
	 ...
	 UartxSendData_UD(uartId, (U8*)READ_SOFTID_CMD, 9);
...
{
	Scan_Param[SCAN_NUM].ScanFlag = flag;
}
...
_Bool WaitScanData_BC(U8 SCAN_NUM,U8 uartId)
{
	...
	SetScanState(SCAN_NUM,0); // 清除扫码完成标志    
	...
	{
		vTaskDelay(200);
		if(Scan_Param[SCAN_NUM].ScanFlag == 1) //扫码完成
		{
			SetScanCodeMode(uartId,MANUAL_SCAN);//关闭扫码
			return 1;
		}
		cnt++;
		if(cnt > 25) //超过5S未收到扫码完成信号，超时退出
			break;
	}
	SetScanCodeMode(uartId,MANUAL_SCAN);//关闭扫码
	return 0; //扫码失败
