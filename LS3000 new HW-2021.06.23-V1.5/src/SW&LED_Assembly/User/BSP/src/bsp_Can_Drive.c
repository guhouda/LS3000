...
static  u8           g_tCanTxBuff[256];            //Can发送数据缓存
static SemaphoreHandle_t	CanTxMutex;						// Can发送互斥信号量
xQueueHandle TestTask1Queue;		//后台一消息队列
xQueueHandle TestTask2Queue;		//后台二消息队列
xQueueHandle QueryTaskQueue;       //信息查询消息队列
...
//创建Can发送消息队列
void createCanMsgTaskQueue(void)
{ 
	CanMsgQueue = xQueueCreate(QUEUE_SIZE,sizeof(Can_MSG)); //消息队列长度1位，存放1个字节
}
...
void CAN_Init_CD(void)
{
	...
	CanTxMutex = xSemaphoreCreateMutex();							    // 创建Can发送互斥信号量
}
...
{	
	...
	TxMessage.ID = Cartridge_Switch_ID;   //通信地址
	...
	for(i = 0;i < (len/4 + 1) ;i++)
	{
		if(i == len/4)
		{
			if(len%4 != 0)
			{
                ...
                xQueueSend(CanMsgQueue,&TxMessage,10);
...
void CAN1_RX0_IRQHandler(void)
{
	...
	if(CAN_GetITStatus(CAN1,CAN_IT_FMP0)!=RESET)
	{
		...
		if(RxMessage.StdId == 0x01 && RxMessage.ExtId != 0x06 && RxMessage.Data[0] == 0x06)//接收上位机数据
		{
            DEBUG("接收到任务命令: ");
            for( U8 i=0; i < 8; i++ )
