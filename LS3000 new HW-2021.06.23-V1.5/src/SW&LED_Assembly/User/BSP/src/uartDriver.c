...
static U8 					g_uart2RxBuff[1024];	    // 串口2接收缓冲区   SCAN2	TX:PA2	RX:PA3   // TX: PD5 ,RX : PD6 
static U8 					g_uart2TxBuff[1024];	    // 串口2发送缓冲区
...
void HandlerComm(UARTProfile *p, U8 uartId)
{
	...
	if (DataLen) // 有数据
	{
        ...
        else if(p->UARTx == USART2) //内扫码
        {
			for(U16 i = 0;i < DataLen; i++)
			{
                if(g_rxBuff[i] == 0x0D && DataLen <= 16 && i > 10) // 接收到一帧数据
				{   
                    memset(Scan_Param[BOX_SCAN4].BarCode,0,sizeof(Scan_Param[BOX_SCAN4].BarCode));                                       
                    memcpy(Scan_Param[BOX_SCAN4].BarCode,g_rxBuff,i);
                    Scan_Param[BOX_SCAN4].BarCode[i] = 0xAA;
                    DEBUG("卡盒四扫码：");                        
                    for(U8 j = 0;j<i;j++)
                    {
                        DEBUG("%c",Scan_Param[BOX_SCAN4].BarCode[j]);
                    }	
                    DEBUG("\r\n");                         
                    SetScanState(BOX_SCAN4,1);	
                    vTaskDelay(10);
...
//				printf("g_rxBuff[i] = %x\r\n",g_rxBuff[i]);
        ...
        else if(p->UARTx == USART3) //内扫码
        {
			for(U16 i = 0;i < DataLen; i++)
			{
                if(g_rxBuff[i] == 0x0D && DataLen <= 16 && i > 10) // 接收到一帧数据
				{   
                    memset(Scan_Param[BOX_SCAN1].BarCode,0,sizeof(Scan_Param[BOX_SCAN1].BarCode));
                    memcpy(Scan_Param[BOX_SCAN1].BarCode,g_rxBuff,i);
                    Scan_Param[BOX_SCAN1].BarCode[i] = 0xAA;
                    DEBUG("卡盒一扫码：");                        
                    for(U8 j = 0;j<i;j++)
                    {
                        DEBUG("%c",g_rxBuff[j]);
                    ...
                    DEBUG("\r\n"); 						
                    SetScanState(BOX_SCAN1,1);
                    vTaskDelay(10);
                    InitQueue_Q(&p->readBuf);                  
                }
            }
        }  
        else if(p->UARTx == UART5) //内扫码
        {
			for(U16 i = 0;i < DataLen; i++)
			{
                if(g_rxBuff[i] == 0x0D && DataLen <= 16 && i > 10) // 接收到一帧数据
				{   
                    memset(Scan_Param[BOX_SCAN2].BarCode,0,sizeof(Scan_Param[BOX_SCAN2].BarCode));
                    memcpy(Scan_Param[BOX_SCAN2].BarCode,g_rxBuff,i);
                    Scan_Param[BOX_SCAN2].BarCode[i] = 0xAA;
                    DEBUG("卡盒二扫码：");                        
                    for(U8 j = 0;j<i;j++)
                    {
                        DEBUG("%c",g_rxBuff[j]);
                    ...
                    DEBUG("\r\n"); 						
                    SetScanState(BOX_SCAN2,1);	 
                    vTaskDelay(10);
                    InitQueue_Q(&p->readBuf);               
                }
            }
        }  
        else if(p->UARTx == USART6) //内扫码
        {
			for(U16 i = 0;i < DataLen; i++)
			{
                if(g_rxBuff[i] == 0x0D && DataLen <= 16 && i > 10) // 接收到一帧数据
				{   
                    memset(Scan_Param[BOX_SCAN3].BarCode,0,sizeof(Scan_Param[BOX_SCAN3].BarCode));
                    memcpy(Scan_Param[BOX_SCAN3].BarCode,g_rxBuff,i);
                    Scan_Param[BOX_SCAN3].BarCode[i] = 0xAA;
                    DEBUG("卡盒三扫码：");                        
                    for(U8 j = 0;j<i;j++)
                    {
                        DEBUG("%c",g_rxBuff[j]);
                    ...
                    DEBUG("\r\n"); 						
                    SetScanState(BOX_SCAN3,1);
                    vTaskDelay(10);
                    InitQueue_Q(&p->readBuf);               
                }
            }
        }         
        else if(p->UARTx == UART8) //内扫码
        {
			for(U16 i = 0;i < DataLen; i++)
			{
                if(g_rxBuff[i] == 0x0D && DataLen <= 16 && i > 10) // 接收到一帧数据
				{   
                    memset(Scan_Param[BOX_SCAN5].BarCode,0,sizeof(Scan_Param[BOX_SCAN5].BarCode));
                    memcpy(Scan_Param[BOX_SCAN5].BarCode,g_rxBuff,i);
                    Scan_Param[BOX_SCAN5].BarCode[i] = 0xAA;
                    DEBUG("卡盒五扫码：");                        
                    for(U8 j = 0;j<i;j++)
                    {
                        DEBUG("%c",g_rxBuff[j]);
...
//初始化条码扫描队列
void Init_Scan_In_Queue(U8 uartId)
{
