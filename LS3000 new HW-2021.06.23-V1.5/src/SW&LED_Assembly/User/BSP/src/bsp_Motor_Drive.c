...
U16 g_initStep[8] = {2000,2000,2000,2000,500,500,500,500};
...
static _Bool MotorPosReset_TD(U8 mId,U8 timeout)
{
	...
	if(mId == MOTOR4)
	{
		SetMotorMoveParam(mId, mp->sensorDir, g_MachineParam.MotorMoveStep.M4_Work_Pos);
		vTaskDelay(500);
		SetStopMotor(mId); 
	}
	if(mId == MOTOR1)
	{
		SetMotorMoveParam(mId, mp->sensorDir, 700);
		vTaskDelay(500);
		SetStopMotor(mId); 
	}	
	if(mId == MOTOR3)
...
// 电机恢复到初始位置 TRUE :OK  FALSE:ERR
static _Bool MotorPosReset(U8 mId)
{
    MULT_MOTOR_PARAM *mp = GetMotorParam_MD(mId);
...
_Bool MotorMove_TD(U8 mId ,S32 movePos,U8 timeout)   
{
	...
	else
	{
		...
		else if (mP->cutPos > movePos)
		{
			...
			{
				 SetMotorRunParam_MD(mId, mP->sensorDir, mP->reg.runspeed);
				 while((GetOptPosSta(mId) == FALSE ) && (MotorTimeOut(mId) == FALSE)) 
				 {
					 vTaskDelay(5);
				 } 
				 SetStopMotor(mId); 
...
//mId ： 电机号  movepos  :电机运行的绝对位置    isRunTogether 是否同时运行  true同时运行   false单个运行 
_Bool MotorMove_MD(U8 mId ,S32 movePos,_Bool isRunTogether)   
{
  ...
  else
  {
	 ...
	 else if (mP->cutPos > movePos)
	 {
			tmppos = mP->cutPos - movePos;
			SetMotorMoveParam(mId, mP->sensorDir, tmppos);		 
			while(GetOptPosSta(mId) == FALSE&&(!GetMotorBusy(mId))) // 
			{
				 vTaskDelay(5);
			} 
			SetStopMotor(mId); 		
