...
static void ConfigPWM(U8 pwm)
{
    ...
    TIM_TimeBaseinit.TIM_Period = 399;
    TIM_TimeBaseinit.TIM_Prescaler = 599; //440-400HZ
...
static void ADD_INPUT_ConfigPWM(U8 pwm)
{
	...
	TIM_TimeBaseinit.TIM_CounterMode = TIM_CounterMode_Up;
	TIM_TimeBaseinit.TIM_Period = 399;
	TIM_TimeBaseinit.TIM_Prescaler = 1199; //440-400HZ
...
{
	U8 id;	
	MACHINE_PARAM* pMP = GetMachineParam_HD();     // 获取系统参数
	// 初始化IO
	InitADC_ADC();	
	// LED_PWM配置
	ConfigPWM(pMP->LedPwm);
	// ADD_INPUT_PWM配置
	ADD_INPUT_ConfigPWM(pMP->LedPwm);	
	DEBUG("LedPwm = %d\r\n",pMP->LedPwm);
	ADS1246_RST(1); 
	delay_nms(2);
	ADS1246_RST(0);
	...
	WriteADS1246Reg(ADS1246_REG_SYS0, ADC_GAIN_4 | ADC_SPS_320);            
	id = ReadADS1246Reg(ADS1246_REG_SYS0);
	if (id != (ADC_GAIN_4 | ADC_SPS_320))
	{
        DEBUG("AD失败");
        ADS1246_START(0); 
        return TRUE;
