...
// 变量定义
static MULT_MOTOR_PARAM g_mtParam[MAX_L6470_NUM];		// 电机配置及运行参数
static SemaphoreHandle_t	SPI4Mutex;					// SPI4通信互斥信号量
static MULT_MOTOR5160_PARAM g_mt5160Param[3];
...
static void IO_CS_Init(void)
{
	...
	| MOTOR_CS6_CLK | MOTOR_CS7_CLK,ENABLE);
...
static void IO_Motor_Init(void)
{
	...
	for(U32 mId = 0; mId < MAX_L6470_NUM; mId++)
	{
		...
		g_mtParam[mId].state     = MOTOR_STATE[mId+3];						// 电机当前状态:是否在运动
		g_mtParam[mId].cutPos    = MOTOR_CUT_POS[mId+3];						// 当前电机所处位置
		g_mtParam[mId].sensorDir = MOTOR_SENSOR_DIR[mId+3];					// 向着复位光耦方向运动的电平
		for(U8 Num = 0; Num < g_mtParam[mId].sensorNum; Num++)				// 初始化电机运动光耦信号IO
		{
			g_mtParam[mId].sensorIO[Num]  = MOTOR_OPT_IO[mId+3];
			g_mtParam[mId].sensorVol[Num] = MOTOR_SENSOR_VOL[mId+3];			// 光耦被挡住时电压
		...
		SendCmdToMotor(mId+3, &g_mtParam[mId]); // 配置每个电机相关运动参数寄存器
	}
    
    memset(&g_mt5160Param, 0, sizeof(MULT_MOTOR5160_PARAM) * 3);
    for(U32 mId = 0; mId < 3; mId++)
    {
	...
	{	
		...
		g_mt5160Param[mId].sensorDir = MOTOR_SENSOR_DIR[mId];					// 向着复位光耦方向运动的电平
		g_mt5160Param[mId].reg56    = MOTOR5160_PARAM_REG[mId];	
        g_mt5160Param[mId].cutPos   = 0;		        
		SendCmdToMotor5160(mId, &g_mt5160Param[mId]); // 配置每个电机相关运动参数寄存器

		for(U8 Num = 0; Num < g_mt5160Param[mId].sensorNum; Num++)				// 初始化电机运动光耦信号IO
		{
			g_mt5160Param[mId].sensorIO[Num]  = MOTOR_OPT_IO[mId];
			g_mt5160Param[mId].sensorVol[Num] = MOTOR_SENSOR_VOL[mId];			// 光耦被挡住时电压
...
{
	MULT_MOTOR_PARAM* p = GetMotorParam_MD(mId);
...
S32 GetAbsPos(U8 mId)
{
	...
	ReadRegData(mId, 0x01, (U8*)&data, 3);
	pos = ((U32)data[0]<<16) | ((U32)data[1]<<8) | data[2];
	printf("第%d电机绝对位置为%d\r\n",mId,pos);
	return pos;
}
...
_Bool GetSensorFlag_MD(U8 mId,U8 sensorID)
{
    ...
    {
        MULT_MOTOR5160_PARAM *p = GetMotor5160Param_MD(mId);   
        U8  state = GPIO_ReadInputDataBit(p->sensorIO[sensorID].mIO, p->sensorIO[sensorID].mPin.GPIO_Pin);
        if (state == p->sensorVol[sensorID]) // 在光耦
            return TRUE;
        else
...
U16 g_initStep[7] = {500,900,500,2000,2000,15000,2000};
...
_Bool MotorMove_TD(U8 mId ,S32 movePos,U8 timeout)   
{
	...
	else
	{
		...
		{
			if(movePos==0&&(mId == MOTOR3 || mId == MOTOR6))
			{
                if(!MotorPosReset_TD(mId,timeout))
                {		
                    return FALSE;
...
_Bool MotorMove_LIQ(U8 mId ,S32 movePos,U8 timeout)   
{
	...
	{
		if (mP->cutPos + movePos < 256000)
		{
            //注册电机超时任务
            MotorTimeReg(mId,timeout);
			tmppos = movePos + mP->cutPos;
            printf("movePos = %d\r\n",movePos);
			SetMotorMoveParam(mId, !mP->sensorDir, movePos);
			if(WaitMotorIdle_TD(mId,timeout) == FALSE)
			{
				SetStopMotor(mId); 
				return FALSE;  
...
{
	u16 MaxSpeed = 0;
	MaxSpeed = MaxSpd_Steps_to_Par(speed);

	SendCmdToReg(mId, MAX_SPEED, (u8*)&MaxSpeed, sizeof(MaxSpeed));
}
...
{
    u16 Acc = 0;
	Acc= AccDec_Steps_to_Par(acc);

	SendCmdToReg(mId, ACC, (u8*)&Acc, sizeof(Acc));    
    SendCmdToReg(mId, DEC, (u8*)&Acc, sizeof(Acc));
}
...
static void IO_Motor5160_Init(void)
{
	...
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;
    //电机1
	GPIO_InitStructure.GPIO_Pin = MOTOR1_ENN_PIN;		
	GPIO_Init(MOTOR1_ENN_PORT, &GPIO_InitStructure );
    ...
    //电机2
	GPIO_InitStructure.GPIO_Pin = MOTOR2_ENN_PIN;		
	GPIO_Init(MOTOR2_ENN_PORT, &GPIO_InitStructure );
    ...
    //电机3
	GPIO_InitStructure.GPIO_Pin = MOTOR3_ENN_PIN;		
	GPIO_Init(MOTOR3_ENN_PORT, &GPIO_InitStructure );
	
	GPIO_InitStructure.GPIO_Pin = MOTOR3_STEP_PIN;		
	GPIO_Init(MOTOR3_STEP_PORT, &GPIO_InitStructure );
	
	GPIO_InitStructure.GPIO_Pin = MOTOR3_DIR_PIN;		
	GPIO_Init(MOTOR3_DIR_PORT, &GPIO_InitStructure );
...
_Bool TMC5160_MotorPosReset(U8 mId)
{
    ...
    if(mId == MOTOR3)
        TMC5160_speedmode(mId,mp->sensorDir,4500);
    else
        TMC5160_speedmode(mId,mp->sensorDir,4000);
    ...
    if(mId == MOTOR1)
    {
        TMC5160_MotorMove(mId,-g_MachineParam.MotorMoveStep.M1_Offset_Steps);
    } 
    if(mId == MOTOR2)
    {
        TMC5160_MotorMove(mId,-g_MachineParam.MotorMoveStep.M2_Offset_Steps);
    }

    if(mId == MOTOR3)
    {
		#if (HARDWARE_REVISION == 1)
			TMC5160_MotorMove(mId,-g_MachineParam.MotorMoveStep.M3_Offset_Steps);
