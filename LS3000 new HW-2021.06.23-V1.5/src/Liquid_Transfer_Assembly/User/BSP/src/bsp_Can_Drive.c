...
static 	CanTxMsg     g_tCan1TxMsg;		          // 用于CAN1发送
static 	CanRxMsg     g_tCan1RxMsg[1024];		         // CAN1接收缓冲
static  u8           g_tCan1TxBuff[2000];            //Can1发送数据缓存

static 	CanTxMsg     g_tCan2TxMsg;		          // 用于CAN2发送
static 	CanRxMsg     g_tCan2RxMsg[512];		         // CAN2接收缓冲
static  u8           g_tCan2TxBuff[128];            //Can2发送数据缓存

//static SemaphoreHandle_t	Can1TxMutex;						// Can1发送互斥信号量
//static SemaphoreHandle_t	Can2TxMutex;						// Can2发送互斥信号量
...
void CAN1_Init_CD(void)
{
	...
	//Can1TxMutex = xSemaphoreCreateMutex();							    // 创建Can发送互斥信号量
}
...
//	CAN_DeInit(CAN2);												// 复位CAN1寄存器
	...
	CAN_FilterInitStructure.CAN_FilterNumber = 14;					// 滤波器序号，0-27，共28个滤波器
	CAN_FilterInitStructure.CAN_FilterMode = CAN_FilterMode_IdMask;	// 滤波器模式，设置ID掩码模式
	CAN_FilterInitStructure.CAN_FilterScale = CAN_FilterScale_32bit;// 32位滤波
	CAN_FilterInitStructure.CAN_FilterIdHigh = 0x0000;				// 掩码后ID的高16bit
	CAN_FilterInitStructure.CAN_FilterIdLow = 0x0000;				// 掩码后ID的低16bit
    ...
    CAN_ITConfig(CAN2, CAN_IT_FMP0, ENABLE);						// CAN FIFO0 消息接收中断使能
	CAN_ITConfig(CAN2, CAN_IT_TME, DISABLE);                        //关闭发送中断
	
	//Can2TxMutex = xSemaphoreCreateMutex();							    // 创建Can发送互斥信号量
}
...
void SendMsg_ToHost(MODE_ADDRESS mode_address,SEND_CMD_TYPE cmd,U8 state, U8* msg,U16 len)
{
	...
	TxMessage.ID = Liquid_Transfer_ID;   //通信地址
...
void CAN1_RX0_IRQHandler(void)
{
	...
	if(CAN_GetITStatus(CAN1,CAN_IT_FMP0)!=RESET)
	{
		...
		if((RxMessage.StdId == 0x01 && RxMessage.ExtId != 0x04 && RxMessage.Data[0] == 0x04) || \
		  ...
		  (RxMessage.StdId == 0x03 && RxMessage.Data[0] == 0x04 && RxMessage.Data[1] == 0x10 && RxMessage.Data[2] == 0xAB))//接收上位机数据
		{           
			/* 入接收队列 */
