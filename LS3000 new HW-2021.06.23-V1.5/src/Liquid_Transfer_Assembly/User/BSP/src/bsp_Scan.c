...
_Bool WaitOutScanData_BC(U8 uartId)
{
	...
	SetOutScanState(0); // 清除扫码完成标志    
	SetScanCodeMode(uartId,CONTINUOUS_SCAN);//开启扫码
	while(1)
	{
		vTaskDelay(200);
		if(OutScan_Param.ScanFlag == 1) //扫码完成
		{
			DEBUG("扫码添加项目完成\r\n");
	...
	SetScanCodeMode(uartId,MANUAL_SCAN);//关闭扫码
	SendMsg_ToHost(SYN_CMD,ADD_ITEMS,3,NULL,4);
	return 0; //扫码失败
}
