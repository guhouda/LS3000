...
static  u8           g_tCanTxBuff[13000];            //Can发送数据缓存
...
{	
	...
	TxMessage.ID = Blending_Cleaning_ID;   //通信地址
...
void CAN1_RX0_IRQHandler(void)
{
	...
	if(CAN_GetITStatus(CAN1,CAN_IT_FMP0)!=RESET)
	{
		...
		if(RxMessage.StdId == 0x01 && RxMessage.ExtId != 0x05 && RxMessage.Data[0] == 0x05)//接收上位机数据
		{
            DEBUG("接收到任务命令: ");
            for( U8 i=0; i < 8; i++ )
