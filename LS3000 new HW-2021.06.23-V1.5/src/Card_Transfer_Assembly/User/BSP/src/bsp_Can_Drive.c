...
{	
	...
	TxMessage.ID = Card_Transfer_ID;   //通信地址
...
void CAN1_RX0_IRQHandler(void)
{
	...
	if(CAN_GetITStatus(CAN1,CAN_IT_FMP0)!=RESET)
	{
		...
		if(RxMessage.StdId == 0x01 && RxMessage.ExtId != 0x03 && RxMessage.Data[0] == 0x03)//接收上位机数据
		{
//            DEBUG("接收到任务命令: ");
//            for( U8 i=0; i < 8; i++ )
...
//            DEBUG("\r\n");
		...
		else if(RxMessage.StdId == 0x04 && RxMessage.ExtId != 0x03 && RxMessage.Data[0] == 0x03)  //接受其他板子数据
		{
//			DEBUG("接收到任务命令: ");
//			for( U8 i=0; i < 8; i++ )
//				DEBUG("%02X", RxMessage.Data[i]);
