...
void HandlerComm(UARTProfile *p, U8 uartId)
{
	...
	if (DataLen) // 有数据
	{
		...
		{	
			for(U16 i = 0; i < DataLen; i++)
			{
				if (g_rxBuff[i] == 0xAA && g_rxBuff[i+9] == 0xA5) // 收到数据头及数据尾
				{
					...
					if(workmsg.cmd == CMD_STOP_AGING || workmsg.cmd == CMD_CLEAN_CARD_ONE)
		...
		else if(p->UARTx == USART2) //内扫码
		{
			for(U16 i = 0;i < DataLen; i++)
			{
                if(g_rxBuff[i] == 0x0D  && g_rxBuff[i+1] == 0x0A&& DataLen <= 13) // 接收到一帧数据
                {
                    memset(Scan_Param.BarCode,0,sizeof(Scan_Param.BarCode));
                    memcpy(Scan_Param.BarCode,g_rxBuff,i);
										Scan_Param.BarCode[DataLen-1] = 0xAA;
                    DEBUG("内扫码: ");                        
                    for(U8 j = 0;j<i;j++)
                    {
                        DEBUG("%c",g_rxBuff[j]);
				...
				else if(g_rxBuff[i] == 0x0D && DataLen <= 13) // 接收到一帧数据
				{
					memset(Scan_Param.BarCode,0,sizeof(Scan_Param.BarCode));
					memcpy(Scan_Param.BarCode,g_rxBuff,i);
					Scan_Param.BarCode[DataLen-1] = 0xAA;
